...
// @param string[] _custom_events : optional (ex: ['reported_by_user', 'submit_comment', ...])
//
// @return yield-siftscience object
//
module.exports = function(_api_key, _version, _custom_events) {
  ...
  // @return promise
  //
  obj.event.custom_event = function(_type, _data) {
  ...
  //
  console.log(typeof(_custom_events));
  if (typeof(_custom_events) === 'array') {
    for (var i = 0, length = _custom_events.length; i < length; i++) {
      var type = _custom_events[i];
      if (typeof(obj.event[type]) === 'undefined') {
        obj.event[type] = function(_data) {
